...
**Vitrine GIS — `0 - vitrine_gis` (2026-09):** schema de *apresentação* read-only pro QGIS. 14 views `vw_gis_*` sobre schemas `7`/`8` (+ join com `2`/`3`), só `geom` + colunas estáveis — sem PII, sem `foto_*`/`gps_*` cru, sem `respostas`/`fotos` jsonb, sem internos de cálculo. Views **definer** (rodam como `postgres`) → sobrevivem à revogação de USAGE em 7/8. `vw_gis_ligacao_sem_dmc` = ligações do cadastro fora de toda DMC projetada (fringe dos limites; ~1970, mediana 73 m da DMC mais próxima).
...
- **Reconciliação DMC (2026-09):** `ZA0200-DMC-06` (REG0000784+REG1496117 → 861 ha) e `ZA0200-DMC-05` (CXA0000927+CXA0000711 → 937 ha) estavam partidas em 2 polígonos de staging cada → fundidas; `dmc_ligacao`/`dmc_resumo`/`vrp_projetada` repontados; `dmc_projetado`/`dmc_resumo` = **15** (= vigente `dmc`); `vrp_projetada.dmc_divergente` zerou; buraco-sliver de 209 m² na DMC-06 tapado. `ZA1004-DMC-*` (outra ZA) não tocada. Índice `dmc_ligacao(matricula)` criado (anti-join da vitrine).
